...
uint8_t LINK_ldcs(UPDI_APP* app,uint8_t address)
{
  ...
  LOG_Print(app->logger, LOG_LEVEL_VERBOSE, MSG_LDCS, address);
...
void LINK_stcs(UPDI_APP* app,uint8_t address, uint8_t value)
{
  ...
  LOG_Print(app->logger, LOG_LEVEL_VERBOSE, MSG_STCS, address);
  PHY_Send(app->port, buf, sizeof(buf));
}
...
bool LINK_Check(UPDI_APP* app)
{
  ...
  {
    LOG_Print(app->logger, LOG_LEVEL_INFO, MSG_UPDI_INIT_OK);
    return true;
  }
  LOG_Print(app->logger, LOG_LEVEL_WARNING, MSG_UPDI_INIT_NOT_OK);
  return false;
}
...
bool LINK_Init(UPDI_APP* app, char *port, uint32_t baudrate, bool onDTR)
{
  ...
  while (err-- > 0)
  {
    ...
    {
      LOG_Print(app->logger, LOG_LEVEL_ERROR, MSG_UPDI_INIT_FAILED);
      return false;
    }
    if ((app->port = PHY_Init(port, baudrate, onDTR)) == NULL)
      return false;
...
uint8_t LINK_ld(UPDI_APP* app, uint16_t address)
{
  ...
  LOG_Print(app->logger, LOG_LEVEL_VERBOSE, MSG_LD, address);
  PHY_Send(app->port, buf, sizeof(buf));
  PHY_Receive(app->port, &response, 1);
  return response;
}
...
bool LINK_st(UPDI_APP* app, uint16_t address, uint8_t value)
{
  ...
  LOG_Print(app->logger, LOG_LEVEL_VERBOSE, MSG_ST, address);
  PHY_Send(app->port, buf, sizeof(buf));
  PHY_Receive(app->port, &response, 1);
...
bool LINK_ld_ptr_inc(UPDI_APP* app, uint8_t *data, uint8_t size)
{
  ...
  LOG_Print(app->logger, LOG_LEVEL_VERBOSE, MSG_LD8_OFFSET);
  PHY_Send(app->port, buf, sizeof(buf));
...
bool LINK_ld_ptr_inc16(UPDI_APP* app, uint8_t *data, uint16_t words)
{
  ...
  LOG_Print(app->logger, LOG_LEVEL_VERBOSE, MSG_LD16_OFFSET);
  PHY_Send(app->port, buf, sizeof(buf));
...
bool LINK_st_ptr(UPDI_APP* app, uint16_t address)
{
  ...
  LOG_Print(app->logger, LOG_LEVEL_VERBOSE, MSG_ST_TO_PTR);
  PHY_Send(app->port, buf, sizeof(buf));
  PHY_Receive(app->port, &response, 1);
  if (response != UPDI_PHY_ACK)
    return false;
...
bool LINK_st_ptr_inc(UPDI_APP* app, uint8_t *data, uint16_t len)
{
  ...
  LOG_Print(app->logger, LOG_LEVEL_VERBOSE, MSG_ST8_OFFSET);
  PHY_Send(app->port, buf, sizeof(buf));
  PHY_Receive(app->port, &response, 1);
  if (response != UPDI_PHY_ACK)
    return false;
...
bool LINK_st_ptr_inc16(UPDI_APP* app, uint8_t *data, uint16_t len) // length in words or in bytes??
{
  ...
  LOG_Print(app->logger, LOG_LEVEL_VERBOSE, MSG_ST16_OFFSET);
  PHY_Send(app->port, buf, sizeof(buf));
  PHY_Receive(app->port, &response, 1);
  if (response != UPDI_PHY_ACK)
    return false;
...
void LINK_Repeat(UPDI_APP* app, uint16_t repeats)
{
  ...
  LOG_Print(app->logger, LOG_LEVEL_VERBOSE, MSG_REPEAT, repeats);
  PHY_Send(app->port, buf, sizeof(buf));
}
...
bool LINK_SendKey(UPDI_APP* app, char *key, uint8_t size)
{
  ...
  LOG_Print(app->logger, LOG_LEVEL_VERBOSE, MSG_WRITING_KEY);
  if (strlen(key) != (8 << size))
  {
    LOG_Print(app->logger, LOG_LEVEL_ERROR, MSG_INVALID_KEY_LEN);
    return false;
  }
  PHY_Send(app->port, buf, sizeof(buf));
  n = strlen(key);
